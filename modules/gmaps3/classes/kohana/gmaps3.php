...
abstract class Kohana_Gmaps3
{
	 ...
	 * @return  object
	 */
	public function add_circle($lat, $lon, $radius = 50000, $strokecolor = '#000000',
		$strokeweight = 2, $strokeopacity = 1, $fillopacity = 0, $fillcolor = '#FF0000')
	{
	...
	public function add_coord($group, $lat, $lon)
	{
		...
		$this->coords[$group][] = array('lat' => $lat, 'lon' => $lon);
	...
	public function add_infowindow($content, $opened = false, $group = 0, $mark_id = false)
	{
		...
		// Add infowindow
		$this->infos[$group][] = array('mark_id' => $mark_id, 'content' => $content,
			'opened' => $opened);
	 ...
	 * @return  object
	 */
	public function add_mark($lat, $lon, $title = null, $draggable = false, $icon = null,
		$shadow = '', $icon_ops = array(), $marker_ops = array())
	{
	 ...
	 * @return  object
	 */
	public function add_polygon_group($group, $strokecolor = '#000000', $strokeweight =
		2, $strokeopacity = 1.0, $fillopacity = 0, $fillcolor = '#FF0000')
	{
		$this->_add_poly_group('polygon', $group, $strokecolor, $strokeweight, $strokeopacity,
	 ...
	 * @return  object
	 */
	public function add_polyline_group($group, $strokecolor = '#000000', $strokeweight =
		2, $strokeopacity = 1.0, $fillopacity = 0, $fillcolor = '#FF0000')
	{
		$this->_add_poly_group('polyline', $group, $strokecolor, $strokeweight, $strokeopacity,
			$fillopacity, $fillcolor);
	 ...
	 * @return  object
	 */
	public function add_rectangle($lat, $lon, $elat, $elon, $strokecolor = '#000000',
		$strokeweight = 2, $strokeopacity = 1, $fillopacity = 0, $fillcolor = '#FF0000')
	{
	...
	public function center($autofit = true, $mark_id = false)
	{
		...
		switch ($type) {
			...
			case 'polylines':
				...
				// Calculate center
				$center = $this->_calculate_center($bounds['lat_max'], $bounds['lat_min'], $bounds['lon_max'],
					$bounds['lon_min']);
	...
	public function get_apilink($only_url = false)
	{
		...
		// Set language
		$query .= '&language=';
		$query .= $this->config->language == 'i18n' ? substr(I18n::lang(), 0, 2) : $this->
			config->language;
	 ...
	 * @return  object
	 */
	protected function _add_poly_group($type, $group, $strokecolor = '#000000', $strokeweight =
		2, $strokeopacity = 1.0, $fillopacity = '0.35', $fillcolor = '#FF0000')
	{
		$group = Inflector::camelize($group);
		...
		$this->polys[$type][$group] = array('strokeColor' => $strokecolor,
			'strokeWeight' => $strokeweight, 'strokeOpacity' => $strokeopacity,
	...
	{
		return array('lat' => ($lat_max + $lat_min) / 2, 'lon' => ($lon_max + $lon_min) /
			2);
	}
	...
	protected function _generate_markers()
	{
		...
		foreach ($this->marks as $k => $mark) {
			...
			if (!empty($mark['icon'])) {
				...
				$icon_size = isset($mark['icon_size']) ? $mark['icon_size'] : $this->config->
					icon_size;
				$shadow_size = isset($mark['shadow_size']) ? $mark['shadow_size'] : $this->
					config->shadow_size;
				$icon_origin = isset($mark['icon_origin']) ? $mark['icon_origin'] : $this->
					config->icon_origin;
				$icon_anchor = isset($mark['icon_anchor']) ? $mark['icon_anchor'] : $this->
					config->icon_anchor;
			...
			if (!empty($mark['shadow']) || $this->config->view_shadow)
				$js .= ",\nshadow: " . (isset($shadow_key) ? $shadow_key : 'baseShadow_' . $this->
					id);
			else
				$js .= ",\nflat: true";
	...
	protected function _generate_rectangles()
	{
		...
		foreach ($this->rectangles as $k => $rectangle) {
			...
			foreach ($rectangle as $option => $value) {
				if ($option != 'lat' && $option != 'lon' && $option != 'elat' && $option !=
					'elon') {
					$js .= "\n$option: ";
					$js .= is_numeric($value) ? $value : "'$value'";
					$js .= ',';
	...
	{
		// Format the URL
		$url = $this->config->geocoding_url . '/json?sensor=' . $this->_b2s($this->
			config->sensor);
		...
		// Set language
		$url .= '&language=';
		$url .= $this->config->language == 'i18n' ? substr(I18n::lang(), 0, 2) : $this->
			config->language;
